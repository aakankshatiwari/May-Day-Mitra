...
if menu == "🏠 Home":
    st.markdown("""
        ...
        💊 Send prescriptions directly to the pharmacy and avoid queues
    """)

# ---- Step 1: Register Patient & Visitor ---- #
elif menu == "📝 Register MDR Card":
    st.header("📝 Register for May Day Rakshak Pass")
...
# ---- Step 3: Book Doctor Appointment ---- #
    ...
    if st.button("Book Appointment"):
        st.success(f"📅 Appointment confirmed with **{selected_doc}** on {appt_date} at {appt_time}")

# ---- Step 4: Upload Prescription ---- #
elif menu == "💊 Upload Prescription":
    st.header("🧾 Upload Prescription for Hospital Pharmacy")

    visitor_id = st.text_input("Visitor ID (linked with MDR card)")
    presc = st.file_uploader("📷 Upload Prescription Image", type=["jpg", "png", "jpeg"])
    ...
    if st.button("🚀 Send to Pharmacy"):
        if visitor_id and presc:
            ...
            with open(path, "wb") as f:
                f.write(presc.read())
            st.success("🧾 Prescription sent to pharmacy. Collect meds after MDR punch.")
        else:
